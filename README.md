...
手机端只需要下载一个公共配置，然后在 Stash 里把示例节点改成真实节点。
...
下载完成后，仍然停留在 Stash 的 `设置` -> `配置文件` 页面：
...
不要手动刷新主配置；如果刷新了，节点可能会恢复成示例
值，需要重新填写一次。

如果维护者更换了你的节点密码或服务器地址，会私下告诉你新的节点信息。你
只需要回到 Stash 的 `设置` -> `配置文件` -> `编辑`，修改 `My-Trojan`
节点。
